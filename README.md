...
== State Changes (e.g. Update of data) == 
  ...
  - On one node in the cluster, a state change occurs on the database.
  - In the database, the wsrep hooks translate the changes to the write-set.
  - dlopen() makes the wsrep provider functions available to the wsrep hooks.
...
=== Installation of the packages (Centos 7) === 

  * We do this on all 3 nodes 
  * <code>
# on each machine do -> in /etc/yum.repos.d/MariaDB.repo
# MariaDB 10.3 CentOS repository list - created 2020-01-05 12:08 UTC
...
=== Installation of the packages (Ubuntu 18.04.) === 
...
# This set the correct repos and updates them locally 
sudo add-apt-repository 'deb [arch=amd64,arm64,ppc64el] https://mirror.herrbischoff.com/mariadb/repo/10.3/ubuntu bionic main'
# 
sudo apt install mariadb-server-10.3  
</code>
  * Note down the IP's of the 3 nodes 
